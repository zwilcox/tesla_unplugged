...
void SoftADJDS311::ledOn(){
	digitalWrite(_led_pin, HIGH);
}

void SoftADJDS311::ledOff(){
	digitalWrite(_led_pin, LOW);
}


void SoftADJDS311::init(){
...
of the ADJD-S311.
*/
int SoftADJDS311::calibrateClear(){
	int gainFound = 0;
	int upperBox=4096;
	int lowerBox = 0;
	int half;
...
*/
int SoftADJDS311::calibrateColor(){
  int gainFound = 0;
  int upperBox=4096;
  int lowerBox = 0;
  int half;
...
capacitor registers.
*/
void SoftADJDS311::calibrateCapacitors(){
...
void SoftADJDS311::writeInt(int address, int gain){
  if (gain < 4096) 
  {
    byte msb = gain >> 8;
    byte lsb = gain;
...
void SoftADJDS311::performMeasurement(){  
...
int SoftADJDS311::readRegisterInt(unsigned char address){
